...
$flash = new \FlashMessage();
	...
	function update_tags($postedData,$id){
		$flash = new \FlashMessage();
		$db = new dbQuery();
		...
		try{
				...
				$db->where("id",$id);
				if($db->update_where('tags',$postedData)){
					$db->trans_commit();
	...
	function delete_tags($id){
		...
		$result=$db->delete_where("tags",$condition);
		if($result){
